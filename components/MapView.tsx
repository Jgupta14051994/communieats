  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      RESTAURANTS.forEach(r => {
        ...
        const circle = L.circle([r.lat, r.lng], { radius: 200, color, fillColor: color, fillOpacity: 0.35, weight: 2 }).addTo(map)
        circle.bindPopup(`
        ...
        `)
        L.marker([r.lat, r.lng]).addTo(map).bindPopup(`<strong>${r.name}</strong>`)
      })
    })
  }, [])

  return <div id="map" className="w-full h-full rounded-2xl" style={{ minHeight: '500px' }} />
}
